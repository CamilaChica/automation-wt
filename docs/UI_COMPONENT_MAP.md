...
| --- | --- | --- | --- |
| `CustomerPortal` — `frontend/src/components/views/CustomerPortal.tsx` | Public customer workspace: catalog search, RFQ submission, compliance PDF upload, PO submission, private-token shipment tracking, and a support mail link. | Local form/search/tracking state. Uploads accepted files through `/api/attachments`, passes returned IDs to `submitCustomerRFQ`, and refreshes a loaded shipment every minute. | **Live integration.** Upload, RFQ, PO, tracking, loading, and error paths are API-backed. |
...
| `AeroProcurementView` — `frontend/src/components/views/AeroProcurementView.tsx` | RFQ queue, source selection cards, compliance summary, procurement action buttons, workload heatmap, lead-time chart, and telemetry panel. | Fetches `getRFQs`; keeps supplier selection within the declared `A \| B \| C` domain and provides visible action feedback. Source cards and metrics remain hardcoded. | **Partially live.** RFQ list and controls are interactive; quote, split PO, escalation, and carrier/map data remain local/demo behavior. |
| `TraceVaultView` — `frontend/src/components/views/TraceVaultView.tsx` | RFQ documentation list, active-document checklist, simulated OCR canvas, certification/rejection/freeze controls, trace history, and metrics. | Fetches `getRFQs`; trace decisions call the authenticated `/api/internal/rfqs/{rfq_id}/trace-decision` endpoint and hard freeze persists automation pause state. | **Partially live.** Decisions and audit records persist; checklist/document/OCR values remain demo data. |
...
| --- | --- | --- | --- |
| High | Customer compliance PDFs were not submitted. | Resolved: portal uploads the file and passes the returned attachment ID with RFQ intake. | Add server-side attachment-to-RFQ relational storage when the production schema is expanded. |
| High | Critical procurement controls are partially local. | Supplier/procurement/fulfillment actions provide visible local feedback; backend command contracts remain incomplete. | Add typed command endpoints for quote assembly, PO issuance, packaging, and stamp generation. |
| High | Internal trace decisions were local only. | Resolved: Trace Vault decisions persist audit records; hard freeze pauses automation. | Add durable document-status columns when OCR/document services are connected. |
...
| Medium | Data integrity/type debt remains. | Shipment/offers/service results still use `any`; supplier selection cast and missing `VITE_ALLOW_MOCK_FALLBACKS` typing were fixed. | Add response interfaces for shipment, offers, attachments, and view-local payloads. |
